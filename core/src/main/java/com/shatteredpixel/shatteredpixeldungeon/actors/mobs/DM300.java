...
public class DM300 extends Mob {

	{
		...
		HP = HT = !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 400 : 300;
		EXP = 30;
		defenseSkill = 15;
	...
	private final int MAX_COOLDOWN = !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 7 : 9;
	...
	public void ventGas( Char target ){
		...
		int gasMulti = !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 2 : 1;
	...
	public void damage(int dmg, Object src) {
		...
		if (dmgTaken > 0) {
			...
			if (lock != null && !isImmune(src.getClass()) && !isInvulnerable(src.getClass())){
				if (!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES))   lock.addTime(dmgTaken/2f);
	...
	public int totalPylonsToActivate(){
		return !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 3 : 2;
	}
	...
	public void supercharge(){
		...
		spend(!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 2f : 3f);
	...
	public static class FallingRockBuff extends DelayedRockFall {
		...
		public void affectChar(Char ch) {
			if (!(ch instanceof DM300)){
				Buff.prolong(ch, Paralysis.class, !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 5 : 3);
				if (ch == Dungeon.hero) {
