...
public class DwarfKing extends Mob {

	{
		...
		HP = HT = !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 450 : 300;
		EXP = 40;
		defenseSkill = 22;
	...
	private final int MIN_COOLDOWN = !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 8 : 10;
	private final int MAX_COOLDOWN = !Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 10 : 14;
	...
	protected boolean act() {
		...
		if (phase == 1) {
			...
			if (summonCooldown <= 0 && summonSubject(!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 2 : 3)){
				summonsMade++;
			...
			if (summonSubject(!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 2 : 3)) summonsMade++;
		}
	...
	public void damage(int dmg, Object src) {
		...
		if (lock != null && !isImmune(src.getClass()) && !isInvulnerable(src.getClass())){
			if (!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES))   lock.addTime(dmg/5f);
		...
		if (phase == 1) {
			...
			if (HP <= (!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 100 : 50)) {
				HP = (!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 100 : 50);
		...
		public void detach() {
			...
			for (Mob m : Dungeon.level.mobs){
				if (m instanceof DwarfKing){
					int damage = m.HT / (!Dungeon.isCheated() && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 18 : 12);
