...
import com.cheatedpixel.cheatedpixeldungeon.levels.painters.CavesPainter;
...
import com.cheatedpixel.cheatedpixeldungeon.sprites.PylonSprite;
...
public class CavesBossLevel extends Level {
	...
	protected boolean build() {
		...
		for (int i= 14*width(); i < length(); i++) {
			if (map[i] == Terrain.EMPTY) {
				if (patch[i - 14*width()]){
					map[i] = Terrain.WATER;
				} else if (Random.Int(!Cheats.isCheated(Cheats.DISABLE_CHALLENGES_EFFECTS) && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 4 : 8) == 0){
	...
	public void eliminatePylon(){
		...
		int finalPylonsRemaining = !Cheats.isCheated(Cheats.DISABLE_CHALLENGES_EFFECTS) && Dungeon.isChallenged(Challenges.STRONGER_BOSSES) ? 1 : 2;
